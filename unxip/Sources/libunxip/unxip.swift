...
public typealias DataReaderStreamDidRead = (_ bytesCount: Int) -> Void
...
extension DataReader where S == AsyncThrowingStream<[UInt8], Error> {
    public init(descriptor: CInt, rootURL: URL, streamingDelegate: DataReaderStreamDidRead?) {
        self.init(data: Self.data(readingFrom: descriptor, rootURL: rootURL, streamingDelegate: streamingDelegate))
	}

    public static func data(readingFrom descriptor: CInt, rootURL: URL, streamingDelegate: DataReaderStreamDidRead?) -> S {
...
public protocol StreamAperture {
    ...
    static func transform(_: Input, rootURL: URL, options: Options?) -> Next.Input
}
...
public enum XIP<S: AsyncSequence>: StreamAperture where S.Element: RandomAccessCollection, S.Element.Element == UInt8 {
    ...
    public static func transform(_ data: Input, rootURL: URL, options: Options?) -> Next.Input {
...
public enum Chunks: StreamAperture {
    ...
    public static func transform(_ chunks: Input, rootURL: URL, options: Options?) -> Next.Input {
		...
		Task {
			...
			while true {
				...
				var name = String(cString: _name)
                if name.hasPrefix("./") {
                    name = rootURL.appendingPathComponent(String(name.dropFirst(2))).path
                }
                var file = File(dev: dev, ino: ino, mode: mode, name: name, rootURL: rootURL)
				...
				while filesize > 0 {
                    ...
                    file.compressedSize += buffer.count
...
public enum Files: StreamAperture {
    ...
    public static func transform(_ files: Input, rootURL: URL, options: Options?) -> Next.Input {
		let options = options ?? Self.defaultOptions
...
public struct Unxip {
    public static func makeStream<Start: StreamAperture, End: StreamAperture>(from start: UnxipStream<Start>, to end: UnxipStream<End>, input: Start.Input, rootURL: URL, _ option1: Start.Options? = nil, _ option2: Start.Next.Options? = nil, _ option3: Start.Next.Next.Options? = nil) -> End.Input where Start.Next.Next.Next == End {
        Start.Next.Next.transform(Start.Next.transform(Start.transform(input, rootURL: rootURL, options: option1), rootURL: rootURL, options: option2), rootURL: rootURL, options: option3)
	}

	public static func makeStream<Start: StreamAperture, End: StreamAperture>(from start: UnxipStream<Start>, to end: UnxipStream<End>, input: Start.Input, rootURL: URL, _ option1: Start.Options? = nil, _ option2: Start.Next.Options? = nil) -> End.Input where Start.Next.Next == End {
        Start.Next.transform(Start.transform(input, rootURL: rootURL, options: option1), rootURL: rootURL, options: option2)
	}

	public static func makeStream<Start: StreamAperture, End: StreamAperture>(from start: UnxipStream<Start>, to end: UnxipStream<End>, input: Start.Input, rootURL: URL, _ option1: Start.Options? = nil) -> End.Input where Start.Next == End {
        Start.transform(input, rootURL: rootURL, options: option1)
	}
